...
- **Operational reporting** — appointment volume, cancellation/no-show rate, by-status breakdown, per-doctor load, and daily volume, all from real `GROUP BY` aggregate queries (never per-row application loops, per the design doc). This needed no new RLS bypass at all: `appointment`'s existing policies already let a department admin see every appointment for doctors in their own department (not just ones they personally touched), and hospital/super admin see everything — so the same RLS-scoped query pattern used everywhere else in this codebase produces correct per-caller numbers automatically. New "Reports" admin page with a date-range picker.
- **Observability** — `logging.pattern.console` has carried a `%X{correlationId}` placeholder since early on, but nothing ever populated it. `CorrelationIdFilter` now runs at `Ordered.HIGHEST_PRECEDENCE` (before Spring Security's own filter chain, so even a permitAll/401/403 request gets one), reuses an incoming `X-Correlation-Id` header or generates one, and echoes it back on the response — verified end-to-end (a correlation ID sent on a booking request shows up on that exact log line, and on the cancellation's log line when cancelling with a different one). Also added the specific logging the design doc calls out by name: booking/reschedule failures (pre-check rejection vs. a lost concurrent-booking race, distinguished explicitly — the second one is the exact scenario the exclusion constraint and the load test exist to prove never produces a double-booking), a security-relevant case that was previously silent (a still-valid token presented for an account suspended *after* the token was issued), and reminder-job claim counts.
- **E4 — admin time-travel toggle.** `TimeTravelClock` (a `Clock` subclass holding a mutable, thread-safe offset) drops straight into the `Clock` bean every service already injects — no scattered refactor needed, which is exactly why the design doc specified the `Clock`-injection discipline "from day one." Restricted to `HOSPITAL_ADMIN`/`SUPER_ADMIN` specifically (narrower than the rest of `/api/admin/**`, per the design doc: "a capability that shifts the whole app's clock warrants the narrowest RBAC tier"). New admin-only "Time Travel" page — deliberately no global banner shown to other roles, per an earlier explicit decision in this project's design review. Fixing this properly surfaced a real gap: `JwtService.verify()` validated token expiration against jjwt's own default clock (real system time), not the app's injected simulated one — meaning a big time jump wouldn't actually affect how "expired" a token looked. Fixed with `JwtParserBuilder.clock(...)`, and verified the fix works: after a 5-day jump, a token issued *before* the jump correctly starts returning 401 (triggering the frontend's existing silent-refresh flow) rather than staying valid for 5 real days.
- **E5 — doctor/department load heatmap** on the Reports page, directly reusing the reporting infrastructure — one more `GROUP BY doctor_id, day` query and a doctor-by-day grid. Deliberately booking *load*, not true free-capacity availability (the design doc phrase "built on the same GROUP BY aggregates already planned for reporting" ruled out re-running slot generation for every doctor's schedule across the whole range, which isn't an aggregate query at all) — the page says so explicitly rather than implying a capability it doesn't have.
- **E2 — public appointment-reference lookup**, no login. Requires reference number *and* last name together (a reference number alone isn't secret enough to gate access) via `lookup_appointment_by_reference` (`V20`) — same narrow SECURITY DEFINER pattern as `V6`/`V10`/`V12`/`V15`/`V18`/`V19`, since there's no `RlsContext` at all for an unauthenticated request. Also added a basic in-memory sliding-window rate limiter (`InMemoryRateLimiter`, 10 attempts / 5 min per IP) — deliberately in-memory rather than Redis-backed, since this app runs single-instance at this scope (documented as a real limitation, not a shortcut: a horizontally-scaled deployment would need a shared store). A wrong last name and an unknown reference number both return the same `404` — distinguishing them would let an attacker confirm a reference number is real without the second factor. Verified end-to-end including the rate limit actually tripping after the 10th attempt from the same IP.
- **E3 — `.ics` calendar attachment** on booking-confirmation emails (and reschedule notices, the same underlying need). `EmailSender` gained a real attachment parameter — `SmtpEmailSender` switched from `SimpleMailMessage` (which can't carry attachments at all) to `MimeMessageHelper`. The `.ics` file uses plain UTC (`Z`-suffixed) timestamps rather than an embedded `Asia/Thimphu` `VTIMEZONE` block — every calendar client converts a `Z` timestamp to the *viewer's own* local timezone for display, which is exactly "timezone-consistent with the UTC-storage decision" without extra complexity. A reschedule's `.ics` reuses the *same* `UID` as the original booking's (the reference number, which already carries forward unchanged across a reschedule) with `SEQUENCE` incremented — a real calendar app updates the existing entry instead of creating a duplicate. Building this surfaced that `endTime` is never actually populated on the in-memory `Appointment` after `save()` (it's DB-trigger-derived, `insertable=false`) — worked around by computing it directly in `BookingService`/`AppointmentLifecycleService` (the same math the trigger does) rather than trying to read it off the entity. Verified end-to-end: booking and reschedule both produce a correctly-sized `.ics` attachment with the right `UID`/`SEQUENCE`/times; cancellations and reminders correctly get no attachment at all.

Eight real bugs were only findable by actually running this against Postgres, not by reading the code — all fixed:
...
- `ReportingRepository.countByDay` cast a native query's `date`-typed column to `java.sql.Date` — the actual driver/Hibernate combination here returns `java.time.LocalDate` directly, so every call threw `ClassCastException`. Notable side-note: like the `ResponseStatusException` case above, this uncaught exception surfaced to the client as a bare, empty `403` rather than a `500` — the second time an unrelated server-side exception has shown up as an unexplained `403` in this app (root cause still not pinned down; if you hit a mystery `403` with an empty body and a valid token, suspect an uncaught exception on the server before suspecting authorization).

Not yet run: the concurrent-booking load test (needs Docker/Testcontainers, not installed on this dev machine); a real click-through of the frontend in an actual browser (no browser-automation tool in this environment — verified via `tsc -b`/`vite build` plus exact request/response-shape matching against the live API instead). Nothing has been deployed. See "Next Steps" in the design doc for the full build order — every item in **MVP Scope** is now built, plus Observability and four of the five scope-expansion items (E2 public reference lookup, E3 `.ics` attachments, E4 admin time-travel, E5 doctor/department heatmap). What's left: E1 (public status/correctness page — genuinely blocked, see below), the load test, and deployment.

**E1 is the one CEO-review item that can't be built honestly right now.** It's specified as showing "booking success rate and the latest load-test run, refreshed by a nightly cron re-run of the load test" — but the load test itself needs Docker/Testcontainers, which isn't installed on this dev machine, so there's no real load-test data to show. Building the page with a fabricated or placeholder number would misrepresent what's actually been proven; building it honestly means either running the real load test first (once Docker's available) or explicitly building a smaller version that shows real booking activity (success/failure counts from actual traffic) while clearly labeling the load-test figure as "not yet run" rather than inventing one.
