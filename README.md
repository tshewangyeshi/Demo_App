...
Four real bugs were only findable by actually running this against Postgres, not by reading the code — all fixed:
...
Not yet built: admin reporting; the notification-sending worker (bookings/cancellations/reschedules still only write to the outbox table, nothing consumes it yet); the reminder job. Not yet run: the concurrent-booking load test (needs Docker/Testcontainers, not installed on this dev machine); a real click-through of the new frontend pages in an actual browser. Nothing has been deployed. See "Next Steps" in the design doc for the full build order.
