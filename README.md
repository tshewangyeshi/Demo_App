...
Not yet run: the concurrent-booking load test (needs Docker/Testcontainers, not installed on this dev machine); a real click-through of the frontend in an actual browser (no browser-automation tool in this environment — verified via `tsc -b`/`vite build` plus exact request/response-shape matching against the live API instead). Nothing has been deployed. See "Next Steps" in the design doc for the full build order — every item in **MVP Scope** is now built; what's left is the CEO-review scope-expansion cherry-picks (public status page, public reference lookup, `.ics` attachments, admin time-travel toggle, doctor/department heatmap), the load test, and deployment.
